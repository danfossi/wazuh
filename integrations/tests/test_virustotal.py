...
def test_process_args():
    ...
    with patch("virustotal.open", mock_open()), \
            ...
            patch('virustotal.generate_msg', return_value=alert_output) as generate_msg, \
        ...
        assert generated_msg==alert_output
        send_msg.assert_called_once_with(alert_output,alert_output['agent'])
...
def test_debug():
    ...
    with patch('virustotal.debug_enabled', return_value=True), \
        ...
        virustotal.debug(alert_output)
        open_mock.assert_called_with(log_file, 'a')
        open_mock().write.assert_called_with(alert_output)
...
def test_send_msg_raise_exception():
    ...
    with patch('requests.post'), \
        ...
        virustotal.send_msg(alert_output,alert_output['agent'])
...
def test_send_msg():
    ...
    with patch('virustotal.SOCKET_ADDR',"./socket.sock"):
        with socket(AF_UNIX, SOCK_DGRAM) as s:
            s.bind("./socket.sock")
            virustotal.send_msg(alert_output, sys_args_template[3])
